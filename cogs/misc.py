...
class Misc(Cog):
    ...
    async def help(self, ctx: discord.ApplicationContext):
        ...
        embed = discord.Embed(title="Hey There!\nMuzocco! doesnt have too many commands yet!",
                            description="Commands that are available for beta update are listed below (bigger desc coming soon!)", color=0x2F3136)
        embed.add_field(
    ...
    async def userinfo(self, ctx: discord.ApplicationContext, user: discord.Member = None):
        ...
        embed.add_field(
            name=f"🔊Playing Status Or Custom Status: {user.activity}", value="Displaying What User Is Playing", inline=False)
        embed.add_field(
    ...
    async def serverinfo(self, ctx: discord.ApplicationContext):
        ...
        embed.add_field(
            name='💬Channels', value=f'{len(ctx.guild.text_channels)} Text | {len(ctx.guild.voice_channels)} Voice', inline=True)
        await ctx.respond(embed=embed)
